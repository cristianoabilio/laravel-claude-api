                                    ...
                                    <div class="row"><div class="col-sm-12">
                                        <table id="datatable" class="table table-bordered dt-responsive table-responsive nowrap dataTable no-footer dtr-inline collapsed" aria-describedby="datatable_info" style="width: 1263px;">
                                            ...
                                            <tr>
                                                ...
                                                <th class="sorting" tabindex="0" aria-controls="datatable" rowspan="1" colspan="1" style="width: 73px;" aria-label="Age: activate to sort column ascending">Link</th>
                                                <th class="sorting" tabindex="0" aria-controls="datatable" rowspan="1" colspan="1" style="width: 73px;" aria-label="Age: activate to sort column ascending">Youtube</th>
                                            ...
                                            <tbody>
                                                @foreach ($usabilities as $item)
                                                    <tr class="odd">
                                                        <td class="">{{ $item->id }}</td>
                                                        <td>{{ Str::limit($item->title, 50, '...') }}</td>
                                                        <td>{{ $item->link }}</td>
                                                        <td><a href="{{ $item->youtube }}" target="_blank">{{ Str::limit($item->youtube, 50, '...') }}</a></td>
                                                        <td><img src="{{ (! empty($item->image)) ? url('upload/usabilities/' . $item->image) : url('upload/no_image.jpg') }}" class="rounded-circle avatar-xxl img-thumbnail float-start" alt="image profile"></td>
                                                        <td>{{ Str::limit($item->description, 50, '...') }}</td>
                                                        <td>
                                                            <a href="{{ route('usabilities.edit', $item->id) }}" class="btn btn-success">Edit</a>
                                                            <a href="{{ route('usabilities.destroy', $item->id) }}" id="delete" class="btn btn-danger">Delete</a>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
